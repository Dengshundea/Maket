    ...
    <div id="menu">
        <el-aside width="210px">
            ...
            <el-menu
                    ...
                    unique-opened
                    router>
                <el-submenu :index="tree.index" v-for="tree in filterTree" :key="tree.index">
                    <!--菜单名称-->
                    <template slot="title">
                        <i :class="tree.cls"></i>
                        <span>{{tree.title}}</span>
                    </template>
                    <el-menu-item :key="item.index" v-for="item in tree.children" :index="item.index">
    ...
    export default {
        ...
        computed: {
            filterTree() {
                ...
                return this.treelist.filter(function (trees) { //三个参数 trees,index,array
                    // 根据用户当角色过滤对应的菜单
                    return trees.roles.includes(_this.currentRole)
                }) // 2-->6
            }
